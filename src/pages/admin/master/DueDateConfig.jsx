...
import { 
  ...
  User, Calendar, ShieldCheck, X, ChevronRight, AlertCircle
} from 'lucide-react';
...
const DueDateConfig = () => {
  ...
  const generateBreadcrumbs = () => {
    ...
    if (paths.length === 0) {
      return (
        <>
          ...
          <ChevronRight size={14} className="text-slate-400" />
          <span className="text-slate-900 font-semibold tracking-tight">Payment Due Date</span>
        </>
      );
    }
    ...
    return paths.map((path, index) => {
      ...
      // Format the path string for display (e.g., "admin-dashboard" -> "Admin Dashboard")
      const displayName = path.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');

      return (
        <React.Fragment key={routeTo}>
          {isLast ? (
             <span className="text-slate-900 font-semibold tracking-tight">Payment Due Date</span> // Override last node for specific title
          ) : (
             <>
  ...
  return (
    <div className="animate-in fade-in duration-500 w-full pb-10 relative">
          ...
          <div className="bg-white border-l-4 border-l-[#155DFC] shadow-xl rounded-[10px] p-4 max-w-md flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                The Global Payment Due Date has been securely updated to <strong className="text-slate-700">{formatDateForDisplay(dueDate)}</strong>. The system will now use this date for all payment deadlines.
              </p>
            </div>
      ...
      <div className="mb-8 border-b border-slate-200 pb-6">
        ...
        <p className="text-slate-500 mt-1 text-sm font-medium">Set the global deadline for university bill payments and submissions.</p>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200 rounded-[10px] shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2">
              ...
              <h2 className="text-xs font-bold text-slate-700 uppercase tracking-widest">Global Deadline</h2>
            </div>
              ...
              <div className="mb-8">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full px-4 py-3.5 bg-slate-50 border-none shadow-sm rounded-[10px] focus:outline-none focus:ring-2 focus:ring-[#155DFC]/20 transition-all font-bold text-slate-900 text-base"
                    required
                  />
                </div>
          ...
          <div className="p-6 bg-amber-50 border border-amber-100 rounded-[10px] flex items-start gap-4 shadow-sm">
            ...
            <div>
              <h3 className="text-sm font-bold text-amber-900 tracking-tight">Compliance Notice</h3>
